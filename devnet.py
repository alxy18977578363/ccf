...
def deviation_loss(y_true, y_pred):
    ...
    ref = tf.constant(0., dtype=tf.float32)
    dev = (y_pred - tf.reduce_mean(y_pred)) / tf.math.reduce_std(y_pred)
    
    # 计算内部点损失和异常点损失
    inlier_loss = tf.square(tf.maximum(dev - ref, 0))
    outlier_loss = tf.square(tf.maximum(ref - dev, 0))
